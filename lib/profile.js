...
function getRepos (user, authToken, options, cb) {
  ...
  setImmediate(function () {
    ...
    var url = stash.protocol + '://' + stash.user.name + ':' + stash.user.password + '@' + stash.host + '/rest/api/1.0/projects/' + user + '/repos'

    request(url, {qs: {start: options.page * options.pageSize, limit: options.pageSize}}, function (er, response, body) {
      if (er) return cb(er)
